...
def run(
    ...
    """Run the weekly pulse graph (P1 ingest + P2 theme/select/compose)."""
    ...
    if result.get("cleaned_path"):
        typer.echo(f"cleaned: {result['cleaned_path']} (reviews={result.get('review_count', 0)})")
...
def stages() -> None:
    ...
    typer.echo("Bundle: pulse (= theme → select → compose → validate) — uses cleaned corpus")
    typer.echo(
